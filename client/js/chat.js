...
const user_image = `<img src="${url_prefix}/assets/img/user.gif" alt="User Avatar">`;
const gpt_image = `<img src="${url_prefix}/assets/img/culog.gif" alt="GPT Avatar">`;
...
const add_pre_welcome_message = async () => {
	message_box.innerHTML += `
            <div class="message">
                ...
                <div class="content">
                    ${markdown.render(
                        "Hello, I'm CurieX, I hope you're curious too! I'm here to assist you in your career path. How can I help you today?"
                    )}
                </div>
            </div>
        `;
...
const ask_gpt = async (message) => {
	try {
		...
		jailbreak = "gpt-evil";//document.getElementById("jailbreak");
		model = "gpt-3.5-turbo";//document.getElementById("model");
		prompt_lock = true;
		window.text = ``;
		...
		const response = await fetch(`${url_prefix}/backend-api/v2/conversation`, {
			...
			body: JSON.stringify({
				api_key: "_XwB7zEJzZxBsxeLktGB0bIBnrdhqRd9xlceE2aXlYQ",
				conversation_id: window.conversation_id,
				action: `_ask`,
				model:"gpt-3.5-turbo-0301gpt-4",//model: model.options[model.selectedIndex].value,
				jailbreak:"gpt-evil",//jailbreak: jailbreak.options[jailbreak.selectedIndex].value,
				meta: {
					id: window.token,
					content: {
						conversation: await get_conversation(window.conversation_id),
						internet_access: true,//document.getElementById("switch").checked,
						content_type: "text",
						parts: [
							{
								content: message,
...
const set_conversation = async (conversation_id) => {
	...
	conversation.items.unshift({
		role: "assistant",
		content: "Hello!, I'm CurieX, you career guidance AI. How can I help you today?",
	});
...
const new_conversation = async () => {
	...
	let post_welcome = {
		role: "assistant",
		content: "Hello, I'm CurieX, hope you are curious too!",
	};

	// If there are previous conversations, add the post-welcome chat
	if (conversation.items.length > 0) {
		conversation.items.push(post_welcome);
	} else {
		conversation.items.push(post_welcome);
	}
...
const load_conversation = async (conversation_id) => {
	...
	let post_welcome = {
		role: "assistant",
		content: "Welcome back!, Let's continue from where you left.",
	};
